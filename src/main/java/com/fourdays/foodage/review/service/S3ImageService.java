...
public class S3ImageService {
	...
	public String upload(MultipartFile image) {
		if (image.isEmpty() || Objects.isNull(image.getOriginalFilename())) {
			throw new S3Exception(ExceptionInfo.ERR_S3_EMPTY_FILE_EXCEPTION);
		}
	...
	private String uploadImage(MultipartFile image) {
		...
		try {
			...
			throw new S3Exception(ExceptionInfo.ERR_S3_IO_EXCEPTION_ON_FILE_UPLOAD);
		}
	}
	...
	private String uploadImageToS3(MultipartFile image) throws IOException {
		...
		ByteArrayInputStream byteArrayInputStream = new ByteArrayInputStream(
			bytes);    // S3 요청할 때 사용할 byteInputStream 생성
		...
		try {
			...
			throw new S3Exception(ExceptionInfo.ERR_S3_PUT_OBJECT_EXCEPTION);
		} finally {
	...
	private void validateFileExtention(String filename) {
		...
		if (lastDotIndex == -1) {
			throw new S3Exception(ExceptionInfo.ERR_S3_NO_FILE_EXTENTION);
		}
		...
		if (!allowedExtentionList.contains(extention)) {
			throw new S3Exception(ExceptionInfo.ERR_S3_INVALID_FILE_EXTENTION);
		}
	}
	...
	public void deleteImageFromS3(String imageAddress) {
		...
		try {
			...
			throw new S3Exception(ExceptionInfo.ERR_S3_IMAGE_DELETE_EXCEPTION);
		}
	}
